                        ...
                        <div class="card mb-4">
                            <div class="card-body py-4">
                                ...
                                <div class="form-group row row-sm mb-0 align-items-center">
                                    ...
                                    {{-- <label class="col-md-1 fs-11">Fundraiser</label>
                                    <div class="col-md-3">
                                        <select class="multiple-select" id="sales" multiple="multiple" name="sales" data-width="50">
                                            @foreach($sales as $item)
                                                <option value="{{$item->kd_sales}}">{{$item->nm_sales}}</option>
                                            @endforeach
                                        </select>
                                    </div> --}}
                                </div>
...
$(function() {
    ...
    $(document).on("click", "#xlsExport", function()
    {
        ...
        action: "{{route('lap.donasiPerFundraiser.xls')}}",
        url: {
                tglDr: $('#tx-tgl-dr').val(),
                tglSd: $('#tx-tgl-sd').val(),
                kdCabang: $('#cabang').val(),
...
function loadData(page,tglDr,tglSd,cabang,program,jenisPeriode,sales){
    $.ajax({
        url:"{{ route('lap.getDonasiPerFundraiser.tbl') }}",
        method:"POST",
