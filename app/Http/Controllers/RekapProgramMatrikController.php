...
class RekapProgramMatrikController extends Controller
{
    public function index(Request $req)
    {
        $app['judul']   = "Rekap Keuangan Berdasarkan Program";
        $app['program'] = MGM::getProgram(false,false);
        return view('pages.rekap_program.rekapkeuanganmatrik', $app);
    }
    ...
    public function get(Request $req)
    {
        ...
        $program    = MGM::getProgram(false,false);
        ...
        <thead>
          <tr class="border-top">
            ...
            foreach($program as $hdrProgram) {
                $output .='<th class="padat-kecil fw-bold angka">'. $hdrProgram->kd_program .'</th>';
            }
        $output .='</tr>
        </thead>
        <tbody>';
        ...
        $q = "select kd_kategori, nm_kategori "
        ." , sum(pendapatan) pendapatan, sum(pengeluaran) pengeluaran "
        ." , sum(pendapatan-pengeluaran) saldo, sum(mutasi_masuk)mutasi_masuk, sum(mutasi_keluar)mutasi_keluar "
        ." from "
        ." ( "
        ." select rf.kd_kategori, rf.nm_kategori,  dtl.kd_program, prg.nm_program, dtl.kd_project, pry.nm_project "
        ." , sum(dtl.kredit) pendapatan, 0 as pengeluaran "
        ...
        ." UNION "
        ." select rf.kd_kategori, rf.nm_kategori, dtl.kd_program, prg.nm_program, dtl.kd_project, pry.nm_project "
        ." , 0 as pendapatan, sum(dtl.debet)  as pengeluaran "
        ." ,  0 as mutasi_masuk, 0 as mutasi_keluar "
        ." from ac_tjurnal jur "
        ...
        ."     , 0 as pendapatan,  0 as pengeluaran "
        ...
        $q    = MGM::getProgram(false,false);
        foreach ($q as $row) {
            $tr .= '
            <tr ' . $kelas_baris_akhir .'>
              <td class="padat-kecil">'. $no .'</td>
              <td class="padat-kecil">'. $row->kd_program .'</td>';
                foreach ($q as $value) {
                    $tr .= '<td class="padat-kecil angka">'. number_format(0,0,',','.') .'</td>';
                }
            $tr .='</tr>';
            $no++;
